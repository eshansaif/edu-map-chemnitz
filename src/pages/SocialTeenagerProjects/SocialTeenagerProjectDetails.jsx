...
const SocialTeenagerProjectDetails = () => {
  const { id } = useParams();
  ...
  useEffect(() => {
    if (locations.length > 0) {
      const single = locations.find(
        (location) => location?.id == id || location?._id == id
      );
      setDetails(single);
    }
  }, [locations, id]);
    ...
    <div className="flex justify-center items-center">
      <div className="card w-4/5 bg-base-100 shadow-xl">
        <div className="card-body">
          <h2 className="card-title">{details?.properties?.TRAEGER}</h2>
          <p>
            Address: {details?.properties?.STRASSE}, {details?.properties?.ORT}{" "}
            - {details?.properties?.PLZ}
          </p>
          <p>Services: {details?.properties?.LEISTUNGEN}</p>
          <p>
            Telephone:{" "}
...
export default SocialTeenagerProjectDetails;
